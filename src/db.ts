...
// In-progress run state (for resuming drills)
interface RunState {
  id?: number;
  deckId: string;
  elapsedMs: number;
...
class DrillDatabase extends Dexie {
  ...
  constructor() {
    ...
    // Version 6: add runState table for in-progress run persistence
...
// Use transaction to prevent race conditions in multi-tab scenarios
...
export async function updateLastRun(
  ...
  return await db.transaction('rw', db.lastRuns, async () => {
...
// In-progress run state functions (for resuming drills)
export interface RunStateRecord {
  elapsedMs: number;
  mistakes: number;
}

export async function getRunState(deckId: string): Promise<RunStateRecord | null> {
  const record = await db.runState.where({ deckId }).first();
  if (!record) return null;
  return { elapsedMs: record.elapsedMs, mistakes: record.mistakes };
}

// Use transaction to prevent race conditions
export async function setRunState(deckId: string, elapsedMs: number, mistakes: number): Promise<void> {
  await db.transaction('rw', db.runState, async () => {
    const existing = await db.runState.where({ deckId }).first();
    if (existing) {
      await db.runState.update(existing.id!, { elapsedMs, mistakes });
    } else {
      await db.runState.add({ deckId, elapsedMs, mistakes });
    }
  });
}

export async function clearRunState(deckId: string): Promise<void> {
  await db.runState.where({ deckId }).delete();
}
